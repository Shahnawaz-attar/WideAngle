                ...
                <div className="bg-[#0a192f] rounded-lg shadow-xl p-8 md:p-12 max-w-6xl mx-auto">
                    <div className="flex flex-col md:flex-row items-center gap-12">
                        <div className="md:w-1/3 text-center">
                            <img src="https://picsum.photos/400/400?random=4" alt="Hassan Hammadeh, CEO" className="w-48 h-48 md:w-64 md:h-64 rounded-full mx-auto shadow-lg border-4 border-[#64ffda]" />
                            <h3 className="text-2xl font-bold mt-6 text-slate-100">Hassan Hammadeh</h3>
                            <p className="text-slate-300 text-lg">Chief Executive Officer</p>
                        </div>
                        <div className="md:w-2/3">
                            <ul className="space-y-3 text-slate-300 text-lg mb-8">
                                {achievements.map((item, index) => (
                                    <li key={index} className="flex items-start">
                                        <svg className="w-5 h-5 text-[#64ffda] mr-3 mt-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path></svg>
                                        <span>{item}</span>
                                    </li>
                                ))}
                            </ul>
                            <h4 className="text-xl font-semibold text-slate-200 mb-4 border-t border-slate-700 pt-4">Key Projects Delivered:</h4>
                             <div className="flex flex-wrap gap-2">
                                {projects.map((project, index) => (
                                    <span key={index} className="bg-[#64ffda]/10 text-[#64ffda] text-sm font-medium px-3 py-1 rounded-full">{project}</span>
                                ))}
                            </div>
                        </div>
                    </div>
